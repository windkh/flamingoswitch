...
{
	0x25B25B6,// (28Bit) Binary: 0010010110110010010110110110 A ON
	0x24DC206,// (28Bit) Binary: 0010010011011100001000000110 A OFF

	0x2717B51,// (28Bit) Binary: 0010011100010111101101010001 B ON
	0x275BADD,// (28Bit) Binary: 0010011101011011101011011101 B OFF

	0xE56BF91,// (28Bit) Binary: 1110010101101011111110010001 C ON
	0xE4D49F5,// (28Bit) Binary: 1110010011010100100111110101 C OFF

	0x65F1C2A,// (28Bit) Binary: 0110010111110001110000101010 D ON
	0x65B67B6,// (28Bit) Binary: 0110010110110110011110110110 D OFF
};
...
void output(unsigned long code, unsigned int length, unsigned int delay, unsigned int* raw)
{
	if (code != 0)
	{
		const char* pCode = code2bin(code, length);
	...
	Switch.decrypt(code, receiverId, value, rollingCode, transmitterId);
	...
	/* Cross check code for testing
	uint32_t reCode = Switch.encrypt(receiverId, value, rollingCode, transmitterId);
	Serial.print(" Recode: 0x");
	Serial.print(reCode, HEX);
	Serial.println("0"); // as the code is 28Bit we add a zero to get a 32Bit number which can be copied into our sketch
	*/
